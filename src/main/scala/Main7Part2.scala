...
object Main7Part2 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val totalCurrentTakenSpace = allSizes("/")
    println("totalCurrentTakenSpace: " + totalCurrentTakenSpace)

    val minimalSpaceToDelete =
      RequiredDiskSpace - (TotalDiskSpace - totalCurrentTakenSpace)
    println("minimalSpaceToDelete: " + minimalSpaceToDelete)

    val smallestDirectoryToDelete =
      allSizes.toList.sortBy(_._2).find(_._2 >= minimalSpaceToDelete).get
    println("smallestDirectoryToDelete: " + smallestDirectoryToDelete)
  }
